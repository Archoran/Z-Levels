...
namespace ZLevels
{
    public class SectionLayer_LowerThings : SectionLayer
    {
        ...
        public void Print(Graphic_LinkedCornerFiller graphic, SectionLayer layer, Thing thing, int curLevel, int baseLevel)
        {
            ...
            DrawPos_Patch.yLevelOffset = -(curLevel - baseLevel);
        ...
        private static readonly AccessTools.FieldRef<Blight, FloatRange> r_SizeRange = AccessTools.FieldRefAccess<Blight, FloatRange>(AccessTools.Field(typeof(Blight), "SizeRange"));
        public void Print(Blight blight, SectionLayer layer, Graphic newGraphic)
        {
            ...
            if (plant != null)
            {
                PlantUtility.SetWindExposureColors(colors, plant);
            }
            else
            {
                colors[0].a = (colors[1].a = (colors[2].a = (colors[3].a = 0)));
            }
            float num = r_SizeRange().LerpThroughRange(blight.Severity);
            if (plant != null)
            {
        ...
        public void Print(Plant plant, SectionLayer layer, Graphic newGraphic, int curLevel, int baseLevel)
        {
            ...
            int num = Mathf.CeilToInt(plant.Growth * (float)plant.def.plant.maxMeshCount);
            if (num < 1)
            {
                num = 1;
            }
            float num2 = plant.def.plant.visualSizeRange.LerpThroughRange(plant.Growth);
            ...
            foreach (int num5 in positionIndices)
            {
                ...
                Printer_Plane.PrintPlane(size: new Vector2(num3, num3), layer: layer, center: center, mat: matSingle, rot: 0f, flipUv: @bool, uvs: null,
                    colors: colors, topVerticesAltitudeBias: 0.1f, uvzPayload: plant.HashOffset() % 1024);
                num4++;
                if (num4 >= num)
                {
                    break;
        ...
        public void Print(ThingWithComps thingWithComps, SectionLayer layer, Graphic newGraphic)
        {
            ...
            {
                thingWithComps.AllComps[i].PostPrintOnto(layer);
            }
        }
