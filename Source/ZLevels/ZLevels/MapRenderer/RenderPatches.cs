...
namespace ZLevels
{
    ...
    public static class DynamicDrawManager_DrawDynamicThings_Patch
    {
        ...
        private static readonly FieldInfo dynamicDrawManager_drawThings = typeof(DynamicDrawManager).GetField("drawThings", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        [HarmonyPostfix]
        public static void DrawSubLevels(DynamicDrawManager __instance, Map ___map, ref bool ___drawingNow)
        {
                ...
                foreach (var map2 in ZTracker.GetAllMaps(___map.Tile))//.OrderBy(x => ZTracker.GetZIndexFor(x)))
                {
                    ...
                    if (curLevel > baseLevel && baseLevel >= 0)
                    {
                        ...
                        try
                        {
                            ...
                            HashSet<Thing> drawThings = (HashSet<Thing>) dynamicDrawManager_drawThings.GetValue(map2.dynamicDrawManager);
                            foreach (Thing thing in drawThings)
                            {
                                if (thing.def.drawerType != DrawerType.None && thing.def.drawerType != DrawerType.RealtimeOnly)
                                {
                                    ...
                                    {
                                        turretGun.DrawTurretTop(curLevel, baseLevel);
                                    }
                                    continue;
                                }
                                ...
                                if (positionTerrain == ZLevelsDefOf.ZL_OutsideTerrain || position2.InBounds(___map) && position2.GetTerrain(___map) == ZLevelsDefOf.ZL_OutsideTerrain)
                                {
                                        ...
                                        || map2.snowGrid.GetDepth(position) <= thing.def.hideAtSnowDepth))
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            {
                                                pawn.DrawPawn(curLevel, baseLevel);
                                            }
                                            else if (thing is Corpse corpse)
                                            {
                                                corpse.DrawCorpse(curLevel, baseLevel);
                                            }
                                            else if (thing.def.projectile == null && !thing.def.IsDoor)
                                            {
                                                ...
                                                if (!graphics.TryGetValue(curLevel, out var graphic) || true)
                                                {
                                                    ...
                                                    {
                                                        var subGraphic = ((Graphic)Graphic_RandomRotated_subGraphic.GetValue(graphicRandomRotated));
                                                        graphic = subGraphic.GetCopy(drawSize);
                                                        graphic.data = subGraphic.data;
                                                    }
                                                    else
                                                    {
                        ...
                        {
                            Log.Error("Exception drawing dynamic things on level " + curLevel + ": " + arg, false);
                        }
                        ___drawingNow = false;
                    }
                }
        ...
        private static readonly MethodInfo comps_PostDraw = typeof(Building_Door).GetMethod("Comps_PostDraw", BindingFlags.NonPublic | BindingFlags.Instance);
        private static void Draw(this Building_Door door, int baseLevel, int curLevel)
        {
            door.Rotation = Building_Door.DoorRotationAt(door.Position, door.Map);
            float num = 1 - ((float)door.TicksTillFullyOpened / (float)door.TicksToOpenNow);
            ...
            comps_PostDraw.Invoke(door, null);
        }

        private static readonly FieldInfo turretGun_top = typeof(Building_TurretGun).GetField("top", BindingFlags.NonPublic | BindingFlags.Instance);
        private static void DrawTurretTop(this Building_TurretGun turretGun, int baseLevel, int curLevel)
        {
            DrawPos_Patch.ChangeDrawPos = true;
            DrawPos_Patch.yLevelOffset = baseLevel - curLevel;
            DrawPos_Patch.zLevelOffset = (baseLevel - curLevel) / turretDrawZOffset;
            ((TurretTop) turretGun_top.GetValue(turretGun)).DrawTurret();
        }

        private static void DrawPawn(this Pawn pawn, int baseLevel, int curLevel)
        {
            DrawPos_Patch.ChangeDrawPos = false;
            var newDrawPos = pawn.DrawPos;
        ...
        private static void DrawCorpse(this Corpse corpse, int curLevel, int baseLevel)
        {
            DrawPos_Patch.ChangeDrawPos = false;
            var newDrawPos = corpse.DrawPos;
            newDrawPos.z += (baseLevel - curLevel) / pawnZOffset;
            newDrawPos.y -= baseLevel - curLevel;
            newDrawPos.y -= 4f;
